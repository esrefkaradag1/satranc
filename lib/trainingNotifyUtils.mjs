...
export function parentPhonesForStudent(student) {
  const target = student.whatsappNotifyTarget ?? student.whatsapp_notify_target ?? 'father';
  const father = String(student.fatherPhone ?? student.father_phone ?? '').trim();
  const mother = String(student.motherPhone ?? student.mother_phone ?? '').trim();
  const parent = String(student.parentPhone ?? student.parent_phone ?? '').trim();
  const extras = Array.isArray(student.contactNumbers ?? student.contact_numbers)
    ? (student.contactNumbers ?? student.contact_numbers)
    : [];

  let ordered = [];
  if (target === 'mother') {
    ordered = [mother, father, parent, ...extras];
  } else if (target === 'both') {
    ordered = [father, mother, parent, ...extras];
  } else {
    ordered = [father, mother, parent, ...extras];
  }
