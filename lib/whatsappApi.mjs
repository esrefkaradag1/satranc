...
function normalizeRegId(value) {
  ...
  if (!raw) return '';
  // Sayısal ise JSON'da number gönder (WaMessage bazı uçlarda string kabul etmiyor)
...
/**
 * Gönderim öncesi aktif cihaz reg_id'yi doğrula / düzelt.
 * Panelde Aktif görünen cihaz ile kayıtlı instanceName uyuşmazsa otomatik günceller.
 */
async function resolveWaMessageRegId(config) {
  let regId = normalizeRegIdString(config.instanceName);
  try {
    const data = await waFetch(config, '/wp/device', { timeoutMs: 20000, retries: 0, allowSoftFail: true });
    const devices = extractDevices(data);
    const pick = pickBestWaMessageDevice(devices, regId);
    if (pick?.regId && pick.connected) {
      if (pick.regId !== regId) {
        config.instanceName = pick.regId;
      }
      return pick.regId;
    }
    if (pick?.regId) return pick.regId;
  } catch {
    /* mevcut reg_id ile devam */
  }
  return regId;
}
...
async function wamessageSendMessage(config, phone, message) {
  ...
  const resolvedRegId = await resolveWaMessageRegId(config);
  if (!resolvedRegId) {
  ...
  try {
    return await wamessageSendNton(config, [{ reg_id: resolvedRegId, target: digits, message: text }]);
  } catch (ntonErr) {
    try {
