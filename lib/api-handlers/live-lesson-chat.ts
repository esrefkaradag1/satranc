...
import {
  appendLiveLessonChatMessage,
  normalizeLiveChatMessageBody,
} from '../liveLessonChatDb.mjs';
...
export default async function handler(req: Req, res: Res) {
  ...
  const normalized = normalizeLiveChatMessageBody(parseBody(req));
  if (normalized.error) {
  ...
  const result = await appendLiveLessonChatMessage(
    sb,
    normalized.record.roomId,
    normalized.record.message,
  );
  if (!result.ok) {
    res.status(result.status ?? 500).json({
      error: result.error || 'Mesaj kaydedilemedi',
      missingColumn: !!result.missingColumn,
    });
    return;
  }

  res.status(200).json({ ok: true, id: normalized.record.message.id });
}
