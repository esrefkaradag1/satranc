import {
  fetchLichessOAuthAccountProfile,
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    const profile = await fetchLichessOAuthAccountProfile(token);
    if (!profile) {
      res.status(502).json({ error: 'Lichess profili alınamadı', connected: true });
      return;
    }
    res.status(200).json({ connected: true, profile, lichessUsername: profile.username });
  } catch (err) {
