...
function isStudentNotificationsEnabled(student) {
...
async function loadWhatsAppConfig(sb, env) {
  ...
  try {
    ...
    return {
      ...
      devicePhone: String(data.device_phone ?? fromEnv.devicePhone).trim(),
      // API key varsa ve enabled null ise açık kabul et
      enabled: data.enabled != null
        ? Boolean(data.enabled)
        : Boolean(String(data.api_key ?? fromEnv.apiKey).trim()),
    };
  } catch {
    return fromEnv;
  }
...
async function loadStudentsAndHomeworks(sb) {
  ...
  const students = (studentsRes.data ?? [])
    .map(dbRowToStudent)
    .filter((s) => s?.id && isStudentNotificationsEnabled(s));
...
async function sendTrainingNotification({
  ...
  // Yarışı önlemek için gönderimden önce claim — çift mesaj engeli
  ...
  if (channel !== 'whatsapp' && channel !== 'both') {
    if (panelSent) await markNotificationSent(sb, student.id, dayIso, kind);
  ...
  for (const phone of phones) {
    ...
    try {
      ...
      sent += 1;
      // Gönderimde düzeltilen reg_id'yi kalıcı kaydet
      if (sb && waConfig.instanceName) {
        try {
          await sb.from('whatsapp_config').upsert({
            id: 'default',
            instance_name: String(waConfig.instanceName),
...
async function processStudentTrainingNotify(sb, waConfig, student, homeworks, dayIso, mode) {
  if (!isStudentNotificationsEnabled(student)) {
    return { studentId: student.id, action: 'inactive_student' };
  }
...
export async function checkStudentTrainingNotify(body, env) {
  ...
  if (!isStudentNotificationsEnabled(student)) {
    return { status: 200, body: { ok: true, skipped: 'inactive_student' } };
  }
