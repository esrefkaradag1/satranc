...
async function fetchChessComPuzzlesForDay(username, dayIso) {
  const trimmed = String(username ?? '').trim().toLowerCase();
  if (!trimmed) return { count: 0, passed: 0, failed: 0 };
  const url = `https://www.chess.com/callback/tactics/stats/${encodeURIComponent(trimmed)}`;
  try {
    const res = await fetch(url, {
      headers: { Accept: 'application/json', 'User-Agent': 'NetChessAcademy/1.0' },
      signal: AbortSignal.timeout(12000),
    ...
    for (const raw of list) {
      const id = Number(raw?.id ?? raw?.puzzleId ?? 0);
      ...
      seen.add(id);
      const ok = Boolean(raw?.is_passed ?? raw?.isPassed ?? raw?.passed ?? (raw?.result === 1));
      if (ok) passed += 1;
      else failed += 1;
    }
    return { count: passed + failed, passed, failed };
