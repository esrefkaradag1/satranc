...
let lastDoneAt = 0;
let backoffUntil = 0;
/** Sıradaki bekleyen iş sayısı — soft istekler aşırı kuyrukta boş dönsün. */
...
/** Tek upstream görevi (gap + fetch + body) üst süresi — kuyruk kilitlenmesin. */
const UPSTREAM_TASK_TIMEOUT_MS = 18_000;
/** Soft istek: kuyruk bu kadar doluysa upstream'e girme, boş dön. */
const SOFT_MAX_PENDING = 6;
...
export async function lichessProxyRequest(apiPath, searchParams, accept = 'application/json', env = process.env) {
  ...
  // Soft istekler dolu kuyruğu / stuck zinciri bekleyip tüm Platform Çek'i kilitlemesin.
  if (softFail && pendingUpstream >= SOFT_MAX_PENDING) {
    const stale = pickStaleFallback(cacheKey, persisted);
  ...
  const upstreamPromise = fetchLichessUpstream(upstreamUrl, {
    headers: { Accept: accept, 'User-Agent': 'NetChessAcademy/1.0' },
    signal: AbortSignal.timeout(15000),
  });
  const upstream = await withTimeout(upstreamPromise, softFail ? 8_000 : 22_000, {
    ok: false,
    status: softFail ? 504 : 504,
    body: '',
    contentType: 'application/json',
    rateLimited: softFail,
  });
...
export async function fetchLichessUpstream(url, init) {
  ...
  const task = async () => {
    ...
    try {
      ...
      return { ok: false, status: 502, body: '', contentType: 'application/json', rateLimited: false };
    } finally {
      lastDoneAt = Date.now();
    }
  };
  pendingUpstream += 1;
  const guarded = () =>
    withTimeout(task(), UPSTREAM_TASK_TIMEOUT_MS, {
      ok: false,
      status: 504,
      body: '',
      contentType: 'application/json',
      ...
      pendingUpstream = Math.max(0, pendingUpstream - 1);
    });
  const next = chain.then(guarded, guarded);
  // Zincir bir görevde sonsuza takılırsa sonraki soft istekler pendingUpstream ile bypass eder;
  // settled olmayan görev yine de finally ile sayacı düşürür (withTimeout).
  chain = next.catch(() => {});
  return next;
}
