...
function parseTableRow(trHtml: string): Record<string, string> | null {
  ...
  // TCK, FIDE ID, Unvan, Ad, Soyad, UKD, Hızlı UHS, Yıldırım UYS, D.Yıl, İl, (ayrıntılar)
...
/**
 * TSF UKD sorgusu. TC varsa yalnızca TC ile arar (soyad eklenmez — TSF AND filtresi bozar).
 */
export async function fetchUkdFromTsfServer(params: {
  ...
  if (!tc && !soyadRaw) {
    return { error: 'tc veya soyad gerekli' };
  }
  if (tc && tc.length !== 11) {
    return { error: 'TC Kimlik No 11 haneli olmalıdır' };
  }
  ...
  if (tc) {
    ...
    form.set('soyad', toAsciiUpper(soyadRaw));
  }
  ...
  if (!res.ok) {
    return { error: 'TSF sunucusu yanıt vermedi' };
  }
