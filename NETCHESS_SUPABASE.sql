-- =============================================================================
-- NETCHESS — TÜM VERİTABANI ŞEMASI (TEK DOSYA)
-- =============================================================================
-- Supabase Dashboard → SQL Editor → bu dosyanın TAMAMINI yapıştır → RUN
--
-- Güvenli: Mevcut tablolar korunur (IF NOT EXISTS / ADD COLUMN IF NOT EXISTS).
-- Yeni kurulum veya eksik kolon/tablo güncellemesi için uygundur.
--
-- Sonrasında Vercel env:
...
ALTER TABLE public.students ADD COLUMN IF NOT EXISTS is_scholarship_student boolean;
...
-- Uygulama attemptToDb → studentid, homeworkid (PostgreSQL küçük harf)
...
ALTER TABLE public.homework_attempts ADD COLUMN IF NOT EXISTS studentid text;
ALTER TABLE public.homework_attempts ADD COLUMN IF NOT EXISTS homeworkid text;
ALTER TABLE public.homework_attempts ADD COLUMN IF NOT EXISTS puzzleid text;
ALTER TABLE public.homework_attempts ADD COLUMN IF NOT EXISTS puzzletitle text;
...
ALTER TABLE public.homework_submissions ADD COLUMN IF NOT EXISTS student_id text;
ALTER TABLE public.homework_submissions ADD COLUMN IF NOT EXISTS homework_id text;
ALTER TABLE public.homework_submissions ADD COLUMN IF NOT EXISTS submitted_at text;

-- Eski kurulumlarda camelCase kolon adları olabilir; indeks yalnızca var olan kolona
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'homework_attempts' AND column_name = 'studentid'
  ) THEN
    CREATE INDEX IF NOT EXISTS idx_homework_attempts_student ON public.homework_attempts (studentid);
  ELSIF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'homework_attempts' AND column_name = 'student_id'
  ) THEN
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'homework_attempts' AND column_name = 'homeworkid'
  ) THEN
    CREATE INDEX IF NOT EXISTS idx_homework_attempts_homework ON public.homework_attempts (homeworkid);
  ELSIF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'homework_attempts' AND column_name = 'homework_id'
  ) THEN
...
CREATE INDEX IF NOT EXISTS idx_live_lesson_updated ON public.live_lesson_state (updated_at DESC);

-- session_media JSON: floorStudentId, handRaisedStudentIds, independentBoardStudentIds,
-- studentBoards, pendingStudentIds, admittedStudentIds, kickedStudentIds, ...
...
-- ── 12. ÇALIŞMA EVENT SOURCING (REC/SYNC) ───────────────────────────────────
...
-- ── 13. RLS (anon okuma; yazma service role ile) ─────────────────────────────
DO $$
DECLARE
  t text;
BEGIN
  FOR t IN SELECT unnest(ARRAY[
    ...
    'chess_study_snapshots','chess_study_presence'
  ])
  LOOP
...
-- ── 14. STORAGE: öğrenci fotoğrafları ───────────────────────────────────────
...
-- ── 15. REALTIME (çalışma senkronu) ─────────────────────────────────────────
DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.chess_studies;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.chess_study_events;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.chess_study_actions;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.chess_study_snapshots;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.chess_study_presence;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- =============================================================================
-- TAMAMLANDI
-- Service role key RLS'i bypass eder (admin yazma + veli imza API).
-- =============================================================================
