...
// lib/leaderboardPeriodCache.ts
var CACHE_TTL_MS = 8 * 24 * 60 * 60 * 1e3;
...
function chessComGamesForDay(monthGames, username, day) {
  ...
  return monthGames.filter(
    (g) => chessComGameInvolvesUser(g, trimmed) && g.end_time && (istanbulDayKey(new Date(g.end_time * 1e3)) === target || localDayKeyFromMs(g.end_time * 1e3) === target)
  ).length;
}
...
function chessComGamesTimeSecondsForDay(monthGames, username, dayIso) {
  const trimmed = username.trim().toLowerCase();
  return monthGames.filter((g) => chessComGameInvolvesUser2(g, trimmed) && chessComGameOnDay(g, dayIso)).reduce((sum, g) => sum + chessComGameDurationSeconds(g), 0);
}
...
// lib/homeworkPlatformUtils.ts
var MAX_DAILY_PLATFORM_PUZZLES = 500;
var MAX_DAILY_TOTAL_PUZZLES = 1e3;
var MAX_DAILY_ACTIVITY_SECONDS = 10 * 3600;
function isImplausibleDayStats(s) {
  return s.chessComPuzzles > MAX_DAILY_PLATFORM_PUZZLES || s.lichessPuzzles > MAX_DAILY_PLATFORM_PUZZLES || s.puzzleSolved > MAX_DAILY_TOTAL_PUZZLES || (s.activityTimeSeconds ?? 0) > MAX_DAILY_ACTIVITY_SECONDS;
}
function dropPuzzleContribution(s) {
  return {
    ...s,
    puzzleSolved: 0,
    puzzlePassed: 0,
...
function mergePlatformDayStats(prevRaw, nextRaw) {
  ...
  return {
    ...
    lichessError: next.lichessError && !lichessKept ? true : prev.lichessError && !lichessKept ? true : void 0,
    chessComError: next.chessComError && !chessKept ? true : prev.chessComError && !chessKept ? true : void 0
  };
}
...
async function fetchMonthlyJson(username, year, month) {
  ...
  const upstream = await fetchChessComUpstream2(url, {}, 15e3);
  if (!upstream.ok) {
...
function mapStudentRow(row) {
  ...
  return { id, lichessUsername: lichessUsername || void 0, chessComUsername: chessComUsername || void 0 };
}
async function runPlatformDaySync(env = process.env, opts) {
  const today = istanbulDayKey();
  const daysToSync = opts?.days?.length ? [...new Set(opts.days.map((d) => d.slice(0, 10)))] : opts?.day ? [opts.day.slice(0, 10)] : [shiftIstanbulDayKey(today, -1), today];
  const day = daysToSync[daysToSync.length - 1] ?? today;
  const sb = supabaseFromEnv2(env);
  if (!sb) {
    return { ok: false, day, days: daysToSync, students: 0, saved: 0, error: "Supabase yap\u0131land\u0131rmas\u0131 eksik" };
  }
  const { data, error } = await sb.from("students").select("id, lichess_username, chess_com_username, chesscom_username");
  if (error) {
    return { ok: false, day, days: daysToSync, students: 0, saved: 0, error: error.message };
  }
  const students = (data ?? []).map((row) => mapStudentRow(row)).filter((s) => Boolean(s));
  let saved = 0;
  for (const syncDay of daysToSync) {
    for (let i = 0; i < students.length; i += CHUNK) {
      ...
      const { data: existingRows } = await sb.from("chess_platform_day_stats").select("student_id, stats, time_seconds").eq("day", syncDay).in("student_id", chunkIds);
      for (const row of existingRows ?? []) {
        const sid = String(row.student_id);
        const stats2 = row.stats;
        const timeSeconds = Number(row.time_seconds) || 0;
        if (stats2 && typeof stats2 === "object") {
          existingByStudent.set(sid, {
            ...stats2,
            activityTimeSeconds: Math.max(stats2.activityTimeSeconds ?? 0, timeSeconds)
          });
        }
      }
      const { stats } = await computePlatformWeekStats(chunk, [syncDay]);
      const rows = Object.entries(stats).map(([studentId, byDay]) => {
        const fresh = byDay[syncDay] ?? {};
...
function dayFromReq(req) {
  ...
  if (typeof body.day === "string") {
    return String(body.day).slice(0, 10);
  }
  return void 0;
}
