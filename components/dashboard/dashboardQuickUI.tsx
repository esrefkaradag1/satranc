...
export const QuickStatCard: React.FC<{
...
}> = ({ icon, value, label, sub, bg, onClick, valueClassName }) => {
  ...
  const valueCls = valueClassName ?? 'text-xl sm:text-2xl font-black tabular-nums leading-none';
  const inner = (
    <>
      ...
      <p className={`relative text-center ${valueCls}`}>{value}</p>
