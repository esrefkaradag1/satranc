...
export function BoardPositionBuilder({
  ...
  const pieceBtn = (p: string) => (
    ...
    >
      <img src={LICHESS_PIECE(p)} alt={p} className="w-9 h-9 lg:w-10 lg:h-10" />
    </button>
  );
  ...
  return createPortal(
    <div className="fixed inset-0 z-[120] bg-[#0d0f12] flex flex-col animate-in fade-in duration-200">
      ...
      <div className="flex-1 min-h-0 flex flex-col md:flex-row overflow-hidden">
        {/* Sol: taş paleti + temel kontroller */}
        ...
        <div className="flex-1 min-h-0 flex flex-col items-center justify-center p-3 md:p-6 min-w-0">
          <div className="w-full max-w-[min(72vh,100%)] aspect-square">
            <ChessBoardFrame
              boardOrientation={orientation}
              className="w-full h-full rounded-xl overflow-hidden border border-white/10 shadow-2xl cursor-crosshair"
        ...
        <aside className="shrink-0 border-t md:border-t-0 md:border-l border-white/10 bg-slate-900/40 p-3 md:p-4 md:w-64 lg:w-72 xl:w-80 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
          <div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Rok Hakları</p>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <p className="text-[11px] text-slate-400 font-semibold">Beyaz</p>
                {([
                  ['K', 'O-O'] as const,
                  ['Q', 'O-O-O'] as const,
                ]).map(([key, label]) => (
                  <label key={key} className="flex items-center gap-2 text-slate-300 text-[12px] cursor-pointer">
                    <input
                      type="checkbox"
              ...
              <div className="space-y-2">
                <p className="text-[11px] text-slate-400 font-semibold">Siyah</p>
                {([
                  ['k', 'O-O'] as const,
                  ['q', 'O-O-O'] as const,
                ]).map(([key, label]) => (
