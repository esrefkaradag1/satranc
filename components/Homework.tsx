...
function weekStatsForDate(
  ...
  studentIds: string[],
  iso: string,
): Record<string, PlatformDayStats | undefined> {
  const out: Record<string, PlatformDayStats | undefined> = {};
  for (const id of studentIds) {
    const stats = weekStats[id]?.[iso];
    if (stats) out[id] = stats;
  }
  return out;
}

function weekTimeForDate(
  weekTime: Record<string, Record<string, number>>,
  studentIds: string[],
  iso: string,
): Record<string, number> {
  const out: Record<string, number> = {};
  for (const id of studentIds) {
    const sec = weekTime[id]?.[iso];
...
const Homework: React.FC = () => {
  ...
  const handleDeleteHomework = useCallback(async (homeworkId: string) => {
    ...
    const ok = await confirmDialog({
      title: 'Ödevi sil',
      message: `"${label}" ödevi kalıcı olarak silinecek. Öğrenci panelinden de kalkar. Emin misiniz?`,
      confirmLabel: 'Sil',
      variant: 'danger',
    });
    if (!ok) return;
  ...
  }, [homeworks, deleteHomework, selectedHwId, showToast, confirmDialog]);
  ...
  /** Program sekmesi listesindeki tüm günlük programların öğrenci birleşimi */
  const allProgramAssignees = useMemo(() => {
    const seen = new Set<string>();
    const out: typeof students = [];
    for (const hw of programHomeworks) {
      for (const s of getAssignees(hw)) {
        if (!targetStudentIds.has(s.id) || seen.has(s.id)) continue;
        seen.add(s.id);
        out.push(s);
      }
    }
    return out;
  }, [programHomeworks, getAssignees, targetStudentIds]);
  ...
  /** Liste: tüm program öğrencileri; detay: seçili ödevin öğrencileri */
  const programPlatformSyncAssignees = useMemo(() => {
    if (panelTab !== 'program') return [];
    if (programAnalysisView === 'list') return allProgramAssignees;
    const hw = programSelectedHw ?? programHomework;
    if (!hw) return allProgramAssignees;
    return getAssignees(hw).filter((s) => targetStudentIds.has(s.id));
  }, [
    panelTab,
    programAnalysisView,
    allProgramAssignees,
  ...
  const programPlatformSyncAssigneeIdsKey = useMemo(
    () => programPlatformSyncAssignees.map((s) => s.id).sort().join(','),
  ...
  useEffect(() => {
    ...
    if (panelTab !== 'program' || programPlatformSyncAssignees.length === 0) return;
    const today = homeworkDayKey();
    const hasLiveActivity = programPlatformSyncAssignees.some((s) => {
      const stats = studentPlatformWeekStats[s.id]?.[today];
      return (stats?.games ?? 0) > 0 || (stats?.puzzleSolved ?? 0) > 0;
    });
  ...
  const viewDatePlatformStats = useMemo(
    () => weekStatsForDate(
      studentPlatformWeekStats,
      programAssigneesForStats.map((s) => s.id),
      viewDate,
    ),
    [studentPlatformWeekStats, programAssigneesForStats, viewDate],
  );

  const viewDatePlatformTimeSeconds = useMemo(
    () => weekTimeForDate(
      studentPlatformWeekTimeSeconds,
      programAssigneesForStats.map((s) => s.id),
      viewDate,
    ),
    [studentPlatformWeekTimeSeconds, programAssigneesForStats, viewDate],
  );
  ...
  const refreshProgramPlatformStats = useCallback(async (opts?: { silent?: boolean }) => {
  ...
  const handleResetStudent = useCallback(async (studentId: string) => {
    const ok = await confirmDialog({
      ...
      message: 'Bu öğrencinin ödev denemeleri sıfırlanacak. Emin misiniz?',
  ...
  }, [selectedHw, homeworks, getAssignees, resetHomeworkAttemptsForStudent, removeHomeworkSubmission, confirmDialog]);
