...
const isAutomaticVcGameOver = (game: any): boolean => {
  return game.isCheckmate() || game.isStalemate() || game.isInsufficientMaterial();
};
...
const StudentStudyView: React.FC<StudentStudyViewProps> = ({
  ...
  /** Hamle bul: kayıtlı bölüm tanımı kullanılır; canlı sync hamleleri çözüm hattına karıştırılmaz. */
  const puzzleSourceChapter = useMemo((): StudyChapter | null => {
    if (!selectedChapter) return null;
    if (!isInteractivePuzzle) return selectedChapter;

    const savedMoves = resolveStudyChapterSolutionMoves(selectedChapter).filter(Boolean);
    if (savedMoves.length > 0) {
      return {
        ...selectedChapter,
        fen: selectedChapter.fen,
        moves: savedMoves,
      };
    }

    const syncMoves = (legacyChapter?.moves ?? []).filter(Boolean);
    if (syncMoves.length === 0) return selectedChapter;
    return {
      ...selectedChapter,
      fen: selectedChapter.fen?.trim() || legacyChapter?.fen || selectedChapter.fen,
      moves: syncMoves,
      seedTree: selectedChapter.seedTree ?? legacyChapter?.seedTree,
    };
  }, [selectedChapter, legacyChapter, isInteractivePuzzle]);

  const effectiveChapter = useMemo(() => {
    if (!selectedChapter) return legacyChapter;
    if (isInteractivePuzzle) return puzzleSourceChapter ?? selectedChapter;
    return legacyChapter ?? selectedChapter;
  }, [selectedChapter, legacyChapter, isInteractivePuzzle, puzzleSourceChapter]);
  ...
  const puzzlePlayNorm = useMemo(() => {
    if (!isInteractivePuzzle || !puzzleSourceChapter) return null;
    return normalizeStudyChapterPuzzle(puzzleSourceChapter);
  }, [
    isInteractivePuzzle,
    puzzleSourceChapter?.id,
    puzzleSourceChapter?.fen,
    puzzleSourceChapter?.moves,
    puzzleSourceChapter?.orientation,
    puzzleSourceChapter?.seedTree,
  ...
  const isVcGameOver = useMemo(() => {
    ...
    try {
      const startFen = isInteractivePuzzle && puzzlePlayNorm
        ? puzzlePlayNorm.startFen
        : (effectiveChapter?.fen || DEFAULT_FEN);
      const game = makeBuilderGame(startFen);
      for (const move of vcHistory) {
        game.move(move);
      }
      return isAutomaticVcGameOver(game);
    } catch {
      return false;
    }
  }, [isInteractivePuzzle, puzzlePlayNorm, effectiveChapter?.fen, vcHistory, vcManualGameOver]);

  const canClaimDraw = useMemo(() => {
    if (isVcGameOver) return false;
    try {
      const startFen = isInteractivePuzzle && puzzlePlayNorm
        ? puzzlePlayNorm.startFen
        : (effectiveChapter?.fen || DEFAULT_FEN);
      ...
      return game.isThreefoldRepetition() || game.isDrawByFiftyMoves();
    } catch {
      return false;
    }
  }, [isInteractivePuzzle, puzzlePlayNorm, effectiveChapter?.fen, vcHistory, isVcGameOver]);

  const vcOutcome = useMemo(() => {
    if (!isVcGameOver || !effectiveChapter) return null;
    const startFen = isInteractivePuzzle && puzzlePlayNorm
      ? puzzlePlayNorm.startFen
      : (effectiveChapter.fen || DEFAULT_FEN);
  ...
  const currentFen = useMemo(() => {
    ...
    if (isInteractivePuzzle) {
      if (!puzzlePlayNorm) return DEFAULT_FEN;
      return fenAtPuzzleMoveIndex(
        puzzlePlayNorm.startFen,
        chapterMovesForUi,
        currentMoveIndex,
      );
    ...
    return fenToCurrentFen(moveListChapter, currentMoveIndex);
  }, [isLiveAnalysis, moveListChapter, currentMoveIndex, isInteractivePuzzle, syncPathFen, puzzlePlayNorm, chapterMovesForUi]);
  ...
  const puzzleSolutionKey = useMemo(
    () => `${puzzlePlayNorm?.startFen ?? ''}|${chapterMovesForUi.join('\u001f')}`,
  ...
  const puzzlePlayFen = useMemo(() => {
    ...
    if (puzzleSetupPreviewFen && hideEngineForStudentPuzzle) {
      return fenAtPuzzleMoveIndex(
        puzzlePlayNorm.startFen,
        chapterMovesForUi,
        currentMoveIndex,
    ...
    return studyBoardFen;
  }, [
    isInteractivePuzzle,
    puzzlePlayNorm,
    studyBoardFen,
    puzzleSetupPreviewFen,
  ...
  const handlePieceDrop = useCallback(({ sourceSquare, targetSquare, piece }: { piece?: any; sourceSquare: string; targetSquare: string | null }) => {
    ...
    if (isInteractivePuzzle && puzzlePlayNorm && totalMoves === 0) {
      showFeedback('wrong', 'Antrenör henüz çözüm hattını kaydetmedi');
      return false;
    }
    setBoardArrows([]);
    setCircleMarks({});
      ...
      try {
        ...
        if (!match.ok) {
          ...
          setChapterMoveAnalysis((prev) => {
            const next = [
              ...prev,
              {
                ...
                played: `${sourceSquare}-${targetSquare}`,
          ...
          logStudyEvent({
            ...
            playedMove: `${sourceSquare}-${targetSquare}`,
            result: 'wrong',
            thinkMs,
          });
          showFeedback('wrong');
        ...
        setLastMoveSquares(lastMoveHighlightFromSan(playFen, playedSan));
        ...
        if (nextIdx >= totalMoves) {
          ...
          recordProgress(
            progressKey ?? '',
            Math.min(99, Math.round((nextIdx / Math.max(1, totalMoves)) * 100)),
          );
          showFeedback('correct');
        }
        return true;
  ...
  useEffect(() => {
    ...
    if (isAutomaticVcGameOver(game) || vcManualGameOver) return;
  ...
  const canDragStudentPiece = useCallback(({ piece }: { piece?: { pieceType?: string } | string }) => {
    ...
    let colorChar = typeof pieceType === 'string' ? pieceType.charAt(0) : '';
    if (colorChar !== 'w' && colorChar !== 'b') {
      const lower = String(pieceType).toLowerCase();
      if (lower.startsWith('white') || lower.startsWith('w')) colorChar = 'w';
      else if (lower.startsWith('black') || lower.startsWith('b')) colorChar = 'b';
    }
  ...
  return (
    <div className="flex flex-col h-full min-h-0 max-h-[100dvh] bg-[#0d0f12] text-slate-200 overflow-hidden font-sans">
      ...
      {false && studentId && selectedStudy && !hideEngineForStudentPuzzle && (
          ...
          <div className="flex-1 overflow-y-auto bg-[#1e293b] border-t border-[rgba(255,255,255,0.05)]">
            {!vsComputer ? (
              ...
              <div className="p-4 flex flex-col h-full">
                {isVcGameOver && (
                  <div className="mb-4 bg-emerald-500/20 border border-emerald-500/50 rounded-lg p-3 text-center shadow-lg flex flex-col items-center justify-center gap-2">
                    <div>
                      <span className="text-emerald-400 font-bold text-sm tracking-wide">
                        {vcOutcome ? vcOutcome.title : 'Oyun Bitti!'}
                      </span>
                      {vcOutcome && (
                        <p className="text-[10px] text-emerald-400/80 mt-1">{vcOutcome.subtitle}</p>
                      )}
                    </div>
                    <button
                      type="button"
                ...
                <div className="flex-1 overflow-hidden flex flex-col">
                            ...
                            <tr key={i} className="group transition-all">
                              ...
                              <td 
                                ...
                                className={`py-2.5 px-2 font-bold transition-all bg-white/[0.02] ${
                                  ...
                                  isWActive ? 'text-white bg-indigo-500/40 shadow-[inset_0_0_10px_rgba(99,102,241,0.3)]' : ''
                                }`}
                              >
                                {vcHistory[i * 2]}
                              </td>
                              <td 
                                ...
                                className={`py-2.5 px-2 font-bold transition-all bg-white/[0.02] rounded-r-lg ${
                                  ...
                                  isBActive ? 'text-white bg-indigo-500/40 shadow-[inset_0_0_10px_rgba(99,102,241,0.3)]' : ''
                                }`}
                              >
                                {vcHistory[i * 2 + 1] || ''}
                              </td>
                  ...
                  {canClaimDraw && (
                    <button 
                      onClick={claimDraw}
                      className="shrink-0 mt-4 py-3.5 bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/50 text-emerald-400 text-xs font-black uppercase tracking-wider rounded-2xl transition-all active:scale-95 flex items-center justify-center gap-2 shadow-lg animate-pulse"
                    >
                      🤝 BERABERLİK İSTE (3 KONUM / 50 HAMLE)
                    </button>
                  )}
