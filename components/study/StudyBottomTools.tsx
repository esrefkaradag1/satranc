...
interface StudyBottomToolsProps {
  ...
  /** Seçili konumdaki yorum metni (sync ağacı + bölüm). */
  currentCommentText?: string;
  currentCommentLabel?: string;
  currentFen: string;
  ...
  onSetPgnTag: (name: string, value: string) => void;
      ...
      <div className="p-3 sm:p-3 min-h-[88px] max-h-[36vh] sm:max-h-72 overflow-y-auto custom-scrollbar">
        ...
        {activeTab === 'tags' && (
          <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2">
            {(() => {
              const rawTags = chapter?.pgnTags?.length
                ? chapter.pgnTags
                : defaultChapterPgnTags(study.title, chapter?.title ?? 'Bölüm');
              const displayTags = filterDisplayPgnTags(rawTags);
              const addable = availableNewPgnTagTypes(rawTags);
              const pgnDisplay = buildStudyBoardPgnDisplay(
                study.title,
                chapter?.title ?? 'Bölüm',
                Math.max(0, (study.chapters?.findIndex((c) => c.id === chapter?.id) ?? 0)),
                rawTags,
              );
              const headerTitle = pgnDisplay.tagsTabTitle;
              return (
                <>
                  <div className="flex items-start gap-2 pb-2 border-b border-white/5">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm font-bold text-white leading-snug break-words">{headerTitle}</h3>
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 mt-2">
                        {onOpenTopics && !readOnly && (
                          <button
                            type="button"
                        ...
                        {study.topicTags.length > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {study.topicTags.map((tag) => (
                              <span key={tag} className="px-2 py-0.5 rounded-full text-[9px] bg-indigo-500/10 text-indigo-400 font-bold border border-indigo-500/20 uppercase">
                                {tag}
                              </span>
                            ))}
                          </div>
                    ...
                    {onToggleLike && (
                      <button
                        ...
                        className={`shrink-0 flex items-center gap-1 px-2 py-1 rounded-lg transition-all ${
                          study.liked ? 'text-red-400 bg-red-500/10' : 'text-slate-500 hover:text-red-400 hover:bg-white/5'
                        }`}
                      >
                        <Heart className={`w-3.5 h-3.5 ${study.liked ? 'fill-current' : ''}`} />
                        <span className="text-[11px] font-bold">{study.likes}</span>
                      </button>
                    )}
                  </div>
                  {!readOnly && !canEditPgnTags && (
                    <p className="text-[10px] text-amber-400/90 font-bold uppercase tracking-wide">
                      Etiketleri düzenlemek için REC modunu açın
                    </p>
                  )}
                  <table className="w-full text-xs border-collapse study__tags">
                    <tbody>
                      {displayTags.map(([name, value]) => (
                        <tr key={name} className="border-b border-white/5">
                          <th className="text-left py-2 pr-3 text-slate-500 font-bold uppercase tracking-wide align-top w-[38%]">{name}</th>
                            ...
                            {!readOnly && canEditPgnTags ? (
                              <input
                                ...
                                onBlur={(e) => {
                                  ...
                                  if (v !== value) onSetPgnTag(name, v);
                                }}
                                onKeyDown={(e) => { if (e.key === 'Enter') (e.target as HTMLInputElement).blur(); }}
                                className="w-full bg-black/30 border border-white/5 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-teal-500/40"
                              />
                            ) : name.endsWith('FideId') && /^\d+$/.test(value) ? (
                              <a href={`https://lichess.org/fide/${value}/redirect`} target="_blank" rel="noreferrer" className="text-teal-400 hover:underline">{value}</a>
                            ) : (
                      ...
                      {!readOnly && canEditPgnTags && addable.length > 0 && (
                        <tr className="border-b border-white/5">
                          <th className="py-2 pr-3 align-top">
                            <select
                              ...
                              className="w-full bg-[#334155] border border-white/10 rounded-lg px-2 py-1.5 text-[10px] font-bold uppercase text-slate-200 focus:outline-none focus:border-teal-500/40"
                            >
                              <option value="">YENİ ETİKET</option>
                              {addable.map((t) => (
                          ...
                          <td className="py-1.5">
                            <input
                              ...
                              spellCheck={false}
                              disabled={!newTagType}
                              placeholder={newTagType ? `${newTagType} değeri…` : ''}
                              onChange={(e) => setNewTagValue(e.target.value)}
                              onBlur={() => {
                                if (newTagType && newTagValue.trim()) {
                                  onSetPgnTag(newTagType, newTagValue.trim());
                                  setNewTagType('');
                                  setNewTagValue('');
                                }
                              }}
                              onKeyDown={(e) => {
                                if (e.key === 'Enter' && newTagType && newTagValue.trim()) {
                                  onSetPgnTag(newTagType, newTagValue.trim());
                                  setNewTagType('');
                                  setNewTagValue('');
                              ...
                              className="w-full bg-black/30 border border-teal-500/30 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-teal-500/50 disabled:opacity-40"
                            />
                          </td>
                        </tr>
                      )}
        ...
        {activeTab === 'comments' && (
          <div className="animate-in fade-in duration-300">
            <p className="text-[10px] text-slate-500 mb-2 font-bold uppercase tracking-wider">
              {currentCommentLabel ?? (currentMoveIndex === 0 ? 'Bölüm başlangıcı yorumu' : `Hamle ${Math.floor(currentMoveIndex / 2) + 1} yorumu`)}
            </p>
            {readOnly ? (
              <div className="w-full min-h-[96px] bg-black/30 border border-white/5 rounded-xl px-4 py-3 text-sm text-slate-200 whitespace-pre-wrap">
                {currentCommentText.trim() || 'Bu hamle için yorum yok.'}
              </div>
            ) : (
              <textarea
                key={`comment-${chapter?.id}-${currentMoveIndex}-${currentCommentText}`}
                rows={4}
                defaultValue={currentCommentText}
                onBlur={e => onSaveComment(e.target.value)}
                placeholder="Yorum yazın… (sağdaki listede seçili hamlede görünür)"
